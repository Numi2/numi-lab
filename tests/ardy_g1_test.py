...
class ARDYG1Test(unittest.TestCase):
    def test_recovery_handoff_retains_authored_landing_and_drops_bad_suffix(
        self,
    ) -> None:
        frames = 13
        joints = np.zeros((frames, 29), dtype=np.float32)
        joints[2:6, 0] = (0.10, 0.30, 0.22, 0.08)
        joints[2:6, 1] = (0.06, 0.15, 0.10, 0.04)
        joints[12, 0] = 0.80
        contacts = np.ones((frames, 4), dtype=np.uint8)
        contacts[2:6, 2:] = 0
        roots = np.zeros((frames, 7), dtype=np.float32)
        roots[:, 6] = 1.0

        class _FixtureKinematics:
            def forward(self, q: np.ndarray) -> dict[str, np.ndarray]:
                ...
                left = np.eye(4)
                return {
                    "right_ankle_pitch_link": right,
                    "left_ankle_pitch_link": left,
                }

        arrays = {
            "joint_positions": joints,
            "root_position_quaternion_xyzw": roots,
            "foot_contacts": contacts,
            "foot_contact_scores": contacts.astype(np.float32),
        }
        with patch(
            "metalrobo.ardy_g1.G1Kinematics.from_urdf",
            return_value=_FixtureKinematics(),
        ):
            prepared, evidence = prepare_single_support_recovery_handoff(
                arrays,
                {"fps": 20.0, "frame_count": frames},
                Path("fixture.urdf"),
                swing_side="right",
            )
        handoff = evidence["execution_handoff"]
        self.assertEqual(handoff["takeoff_frame"], 2)
        self.assertEqual(handoff["landing_frame"], 6)
        self.assertEqual(handoff["selected_frame"], 10)
        self.assertEqual(prepared["joint_positions"].shape, (11, 29))
        self.assertEqual(handoff["poses_inserted"], 0)
        self.assertEqual(handoff["poses_modified"], 0)

    def test_native_contact_modes_preserve_model_confidence(self) -> None:
        modes, confidence = foot_contact_contract(
            np.asarray(((1, 0, 0, 0), (0, 0, 1, 1)), dtype=np.uint8),
            np.asarray(
        ...
        np.testing.assert_allclose(confidence, ((0.8, 0.02), (0.0, 0.6)), atol=1.0e-7)
    ...
    def test_native_joint_frame_conversion_round_trips_hinge_angles(self) -> None:
        ...
        for output, (mechanism, axis, _) in enumerate(_JOINT_FRAME_CONTRACT):
            ...
            local[name_to_index[_G1_SKEL_JOINTS[output]]] = frame_offset.T @ adjusted
        global_rotations = np.empty_like(local)
        for joint, parent in enumerate(_G1_PARENTS):
            global_rotations[joint] = (
                local[joint] if parent < 0 else global_rotations[parent] @ local[joint]
            )
        rotations_6d = np.concatenate(
            (global_rotations[:, :, 0], global_rotations[:, :, 1]), axis=1
        )[None]
        qpos = _qpos_from_motion(np.zeros((1, 3)), rotations_6d, _G1_NAMES, _G1_PARENTS)
    ...
    def test_mechanism_projection_preserves_source_and_enforces_limits(self) -> None:
        ...
        velocity_ratio = (
            np.abs(np.diff(projected, axis=0)) * 25.0 / _G1_JOINT_VELOCITY[None]
        )
