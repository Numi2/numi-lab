...
is documented in `NUMI_HUMAN_GLOBAL_RECRUITMENT_V6.md`. The Apple M4 Pro
960-sweep global-polish rerun closes body weight to `2.03e-9` relative error and replays
bitwise, but internal balance remains false:

- normalized residual RMS: `0.693308649197`;
- 27 coordinates exceed `1 rad/s2`, two exceed `10 rad/s2`, and none exceed
  `100 rad/s2`;
- the largest errors are bilateral wrist flexion at `19.18/15.78 rad/s2`;
- the next largest errors are left third-MCP flexion (`-8.56`), right thumb CMC
  flexion (`-7.35`), left CMC abduction (`6.59`), right fourth-MCP flexion
  (`6.47`), and left thumb CMC flexion (`-6.39`); and
...
`docs/media/numi-human-global-recruitment-v6/`; earlier all-DoF states
