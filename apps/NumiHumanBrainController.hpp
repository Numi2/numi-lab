        ...
        explicit Library(const std::string& path) {
            ...
            try {
                ...
                motorTissuePhased = optionalSymbol<decltype(motorTissuePhased)>(
                    "nb_human_standing_encode_motor_tissue_phased_v1");
                    ...
                    if (phaseTiming != nullptr && std::strcmp(phaseTiming, "1") == 0) {
                        ...
                        if (splitInner) {
                            if (owner.library_.motorTissuePhased == nullptr) {
                                owner.fail("NumiBrain inner phase timing symbol is unavailable"); return false;
                            }
                            tissueSuccess = owner.library_.motorTissuePhased(
                                owner.brain_.handle, pass.commandBuffer, pass.mujocoStates,
                                416u, error.data(), error.size());
                        } else {
                            ...
                            tissueSuccess = owner.library_.motorTissue(
    ...
    static bool encodeAccepted(void* context, void* commandBuffer) noexcept {
        ...
        @autoreleasepool {
            try {
                ...
                const char* phaseTiming = std::getenv("NUMI_HUMAN_BRAIN_PHASE_TIMING");
                if (phaseTiming != nullptr && std::strcmp(phaseTiming, "1") == 0) {
                    if (owner.library_.acceptedFast == nullptr ||
                    ...
                    const auto success = owner.library_.accepted(owner.brain_.handle,
                        (__bridge void*)encoder, owner.physicalFingerprint_, owner.step_ + 1u,
                        records.data(), static_cast<std::uint32_t>(records.size()),
                        error.data(), error.size());
                    [encoder endEncoding];
                    if (success != 1u) {
