...
struct NumiHumanTendonTractionResult {
    ...
    std::array<std::array<double, 3>, 3> nodalForces{};
    double forceResidual = 0.0;
    double momentResidual = 0.0;
};

// Decode NHTENDON1. expected hashes may be empty to skip identity comparison;
// production callers pass both source identities from NHRIGID2/NHMYO1.
